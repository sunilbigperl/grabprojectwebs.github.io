...
if(next.length){scrollPage(next,null,!1)}};FP.silentMoveTo=function(sectionAnchor,slideAnchor){FP.setScrollingSpeed(0,'internal');FP.moveTo(sectionAnchor,slideAnchor);FP.setScrollingSpeed(originals.scrollingSpeed,'internal')};FP.moveTo=function(sectionAnchor,slideAnchor){var destiny=getSectionByAnchor(sectionAnchor);if(typeof slideAnchor!=='undefined'){scrollPageAndSlide(sectionAnchor,slideAnchor)}else if(destiny.length>0){scrollPage(destiny)}};FP.moveSlideRight=function(section){moveSlide('next',section)};FP.moveSlideLeft=function(section){moveSlide('prev',section)};FP.reBuild=function(resizing){if(container.hasClass(DESTROYED)){return}
...
else if(isResponsive){FP.setAutoScrolling(originals.autoScrolling,'internal');FP.setFitToSection(originals.autoScrolling,'internal');$(SECTION_NAV_SEL).show();$body.removeClass(RESPONSIVE)}};if($(this).length){init();bindEvents()}
...
enableYoutubeAPI();enableVidemoAPI();if(options.scrollOverflow){if(document.readyState==='complete'){createSlimScrollingHandler()}
...
function addInternalSelectors(){container.find(options.sectionSelector).each(function(){$(this).addClass(SECTION)});container.find(options.slideSelector).each(function(){$(this).addClass(SLIDE)})}
...
function enableYoutubeAPI(){container.find('iframe[src*="youtube.com/embed/"]').each(function(){var sign=getUrlParamSign($(this).attr('src'));$(this).attr('src',$(this).attr('src')+sign+'enablejsapi=1')})}
function enableVidemoAPI(){container.find('iframe[src*="player.vimeo.com/"]').each(function(){var sign=getUrlParamSign($(this).attr('src'));$(this).attr('src',$(this).attr('src')+sign+'api=1')})}
...
stopMedia(v.activeSection);element.addClass(ACTIVE).siblings().removeClass(ACTIVE);lazyLoad(element);options.scrollOverflowHandler.onLeave();canScroll=!1;setState(slideIndex,slideAnchorLink,v.anchorLink,v.sectionIndex);performMovement(v);lastScrolledDestiny=v.anchorLink;activateMenuAndNav(v.anchorLink,v.sectionIndex)}
...
function afterSectionLoads(v){continuousVerticalFixSectionOrder(v);$.isFunction(options.afterLoad)&&!v.localIsResizing&&options.afterLoad.call(v.element,v.anchorLink,(v.sectionIndex+1));options.scrollOverflowHandler.afterLoad();playMedia(v.element);v.element.addClass(COMPLETELY).siblings().removeClass(COMPLETELY);canScroll=!0;$.isFunction(v.callback)&&v.callback.call(this)}
...
function playMedia(destiny){var destiny=getSlideOrSection(destiny);destiny.find('video, audio').each(function(){var element=$(this).get(0);if(element.hasAttribute('data-autoplay')&&typeof element.play==='function'){element.play()}});destiny.find('iframe[src*="youtube.com/embed/"]').each(function(){var element=$(this).get(0);if(/youtube\.com\/embed\//.test($(this).attr('src'))&&element.hasAttribute('data-autoplay')){element.contentWindow.postMessage('{"event":"command","func":"playVideo","args":""}','*')}})}
function stopMedia(destiny){var destiny=getSlideOrSection(destiny);destiny.find('video, audio').each(function(){var element=$(this).get(0);if(!element.hasAttribute('data-keepplaying')&&typeof element.pause==='function'){element.pause()}});destiny.find('iframe[src*="youtube.com/embed/"]').each(function(){var element=$(this).get(0);if(/youtube\.com\/embed\//.test($(this).attr('src'))&&!element.hasAttribute('data-keepplaying')){$(this).get(0).contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}','*')}})}
function getSlideOrSection(destiny){var slide=destiny.find(SLIDE_ACTIVE_SEL);if(slide.length){destiny=$(slide)}
return destiny}
function scrollToAnchor(){var value=window.location.hash.replace('#','').split('/');var section=decodeURIComponent(value[0]);var slide=decodeURIComponent(value[1]);if(section){if(options.animateAnchor){scrollPageAndSlide(section,slide)}else{FP.silentMoveTo(section,slide)}}}
...
iScrollInstance=new IScroll($this.get(0),iscrollOptions);iscrollHandler.iScrollInstances.push(iScrollInstance);$this.data('iscrollInstance',iScrollInstance)})},isScrolled:function(type,scrollable){var scroller=scrollable.data('iscrollInstance');if(!scroller){return!0}
